    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                # Сохранить в файл
                output_file = Path(__file__).parent.parent / "docs" / f"auction_lots_example_{region}_{item_id}.json"
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                # Сохранить в файл
                output_file = Path(__file__).parent.parent / "docs" / f"auction_history_example_{region}_{item_id}.json"
                output_file.write_text(json.dumps(data, indent=2, ensure_ascii=False))
                print(f"\nSaved to: {output_file}")
            else:
                print(f"Error: {response.text}")
...
if __name__ == "__main__":
    ...
    ITEM_ID = "example_item_id"  # Заменить на реальный ID предмета
