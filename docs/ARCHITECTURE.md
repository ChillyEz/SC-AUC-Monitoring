...
#### 3.5. Items Database Cache (`app/services/items_database_manager.py`)
- **Роль**: Локальный кэш предметов из stalcraft-database
- **Компоненты**:
  - `ItemsDatabaseManager` - менеджер кэша предметов
  - Singleton instance: `items_db_manager`
- **Особенности**:
  - Автоматическое скачивание и индексация при первом запуске
  - Автообновление каждые 24 часа
  - In-memory индекс для мгновенного поиска (< 10ms)
  - Поддержка нескольких realms (RU, Global)
  - Concurrent downloads с semaphore (max 10)
  - Кэш в JSON формате: `backend/data/items_cache/`
- **Методы**:
  - `initialize()` - загрузка из кэша или скачивание
  - `update_database()` - принудительное обновление
  - `search()` - быстрый поиск по локальной базе
  - `get_item_by_id()` - получение предмета по ID
